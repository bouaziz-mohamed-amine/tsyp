...
export default function ChallengeListItem({ data, classNames, imgClassNames }) {
	return (
		...
		<Link to={data.link}
		target="_blank" class="flex flex-col items-center  border border-gray-200 dark:border-gray-700  rounded-xl border-2 shadow md:flex-row  hover:bg-gray-100 dark:hover:bg-black">
	...
	<div class="flex flex-col justify-between p-4 leading-normal w-full Link">
        <h4 class="mb-2 text-center text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">{data.name}
		</h4>
        <p class="mb-3 text-center  font-bold text-gray-600 dark:text-gray-400">{data.slug}</p>
    </div>
</Link>
			</div>
